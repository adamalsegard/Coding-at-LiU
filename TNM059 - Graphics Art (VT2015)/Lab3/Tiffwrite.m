function status = tiffwrite(arg1, arg2, arg3, arg4, arg5, arg6)
...
%
%	TIFFWRITE(C,M,Y,K,'filename') writes a TIFF file containing the CMYK
%	image in the matrices C,M,Y,K to a disk file called 'filename'.
%
...
%
%	TIFFWRITE(R,G,B,'filename',['compressflagg',] '48bit') writes a
%	TIFF file containing the 48 bit RGB image in the matrices R,G,B to
%	a disk file called 'filename'. The extension '.tif' will be added
%	to 'filename' if it doesn't already have an extension.
%
...
%       Modified by Jorgen Rydenius to handle 48-bit RGB images and CMYK images.
...
%TYPES: 1 - gray, 2 - RGB, 3 - RGB palette, 
%       5 - CMYK, 8 - CIELab, 255 - RGB(48-bit, virtual type number)
...
elseif nargin==4
  if isstr(arg3) == 0
    % tiffwrite(R,G,B,'filename') - 24-bit RGB
    type = 2;
...
elseif (nargin==5)
  if ~isstr(arg4)
    % tiffwrite(C,M,Y,K,'filename) - CMYK image
    type = 5;
    draw = zeros(size(arg1,1),4*size(arg1,2));
    draw(:,1:4:width*4) = round(arg1*255);
    draw(:,2:4:width*4) = round(arg2*255);
    draw(:,3:4:width*4) = round(arg3*255);
    draw(:,4:4:width*4) = round(arg4*255);
    filename = arg5;
    comp_flag = 'nocompress';
  elseif strcmp(arg5, 'compress') | strcmp(arg5, 'nocompress') | strcmp(arg5, 'auto')
    % tiffwrite(R,G,B,'filename,'compress_flag') - Compressed 24-bit RGB
    type = 2;
    draw = zeros(size(arg1,1),3*size(arg1,2));
    draw(:,1:3:width*3) = round(arg1*255);
    draw(:,2:3:width*3) = round(arg2*255);
    ...
    filename = arg4;
    comp_flag = arg5;
  elseif strcmp(arg5, '48bit'),
    % tiffwrite(R,G,B,'filename,'48bit') - 48-bit RGB
    type = 255;
    draw = zeros(size(arg1,1),6*size(arg1,2));
    temp = round(arg1*65535);
    draw(:,1:6:width*6) = floor(temp/256);
    draw(:,2:6:width*6) = temp-256*floor(temp/256);
    temp = round(arg2*65535);
    draw(:,3:6:width*6) = floor(temp/256);
    draw(:,4:6:width*6) = temp-256*floor(temp/256);
    temp = round(arg3*65535);
    draw(:,5:6:width*6) = floor(temp/256);
    draw(:,6:6:width*6) = temp-256*floor(temp/256);
    filename = arg4;
    comp_flag = 'nocompress';
  end
elseif (nargin==6),  
  if ~isstr(arg4)
    % tiffwrite(C,M,Y,K,'filename,'compress_flag') - Compressed CMYK image
    type = 5;
    draw = zeros(size(arg1,1),4*size(arg1,2));
    draw(:,1:4:width*4) = round(arg1*255);
    draw(:,2:4:width*4) = round(arg2*255);
    ...
    % tiffwrite(R,G,B,'filename,'compress_flag', '48bit') - Compressed 48-bit RGB
    type = 255;
    draw = zeros(size(arg1,1),6*size(arg1,2));
    temp = round(arg1*65535);
    draw(:,1:6:width*6) = floor(temp/256);
...
%   RGB images are 8 or 16 bits per sample
%   CMYK images are 8 bits per sample
bits = 8;
...
if ((type == 1) | (type == 2) | (type == 5) | (type == 255))
...
elseif (type == 5)
  fwrite(file,[8 8 8 8],'ushort');
elseif (type == 255)
  fwrite(file,[16 16 16],'ushort');
end
%
