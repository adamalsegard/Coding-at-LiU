...
void TNMVolumeInformation::process() {
	...
	if (_data == 0) {
		...
		_data->valueNames.resize(4);
		_data->valueNames[0] = "Intensity";
		_data->valueNames[1] = "Average";
		_data->valueNames[2] = "Standard Deviation";
		_data->valueNames[3] = "Gradient Magnitude";
	}
    ...
    _data->data.resize(dimensions.x * dimensions.y * dimensions.z);
    ...
    for (size_t iX = 0; iX < dimensions.x; ++iX) {
        for (size_t iY = 0; iY < dimensions.y; ++iY) {
            for (size_t iZ = 0; iZ < dimensions.z; ++iZ) {
				...
				_data->data.at(i).dataValues[0] = intensity;
				...
				_data->data.at(i).dataValues[1] = average;
				...
				_data->data.at(i).dataValues[2] = stdDeviation;
				...
				_data->data.at(i).dataValues[3] = gradientMagnitude;
            }
        }
    }
