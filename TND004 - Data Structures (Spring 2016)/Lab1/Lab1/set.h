...
private:
    struct Node
    {
        ...
        explicit Node(const T &value, const shared_ptr<Node> & prev, const shared_ptr<Node> & next)
            : value(value), prev(prev), next(next) {}

        explicit Node(const T &&_value, const shared_ptr<Node> & prev, const shared_ptr<Node> & next)
            : value(_value), prev(prev), next(next) {}
    };
...
public:
    ...
    /**
     * Inline shenanigans to enable implicit conversion in template function
     **/
    ...
    friend const Set operator+ (Set s1, const Set &s2)
    {
        Set s(s1);
